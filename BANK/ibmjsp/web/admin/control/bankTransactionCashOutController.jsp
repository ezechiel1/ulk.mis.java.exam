...
//Table Name
String tableName="bank_account_transaction_cashout";
...
if(request.getParameter("validate").equals("Register"))
{
    ...
    if( transaction_amount < 100 || transaction_amount > 1000000)
    {
      ...
      %>
        <script>window.location.href="http://localhost:81/ibm/admin/execute.jsp?request=bank_transactions_out";</script>
      <% 
    }
    // If Account  Number Exists 
    else if(resultCheckIfAccountNumberExists.next())
    {
        // Currenct Account Balance
        ...
        // Check If Enough Balance Before Cashout
        if( previous_balance > transaction_amount ){

            // Auto generate transaction Id 
            String transaction_id = "IBM_TrX_OUT_00" + timestamp;
            ...
            { 
                ...
                { 
                    ...
                    session.setAttribute("notification_msg","New Customer Bank Account Transaction Cash Out has been recorded successfully!");
                    %>
                      <script>window.location.href="http://localhost:81/ibm/admin/execute.jsp?request=bank_transactions_out"; </script>
                    <%  
                }
            ...
            session.setAttribute("notification_status","Error");
            session.setAttribute("notification_msg","Insuffient Balance. Can not complete this cashout!");
            %>
                <script>window.location.href="http://localhost:81/ibm/admin/execute.jsp?request=bank_transactions_out";</script>
            <%  
        }
